...
# Dynamic Cache to prevent PGRST204 errors
DB_COLS = {}
...
def literal_sync(table_name, df, p_key):
    """Syncs DataFrame to DB with literal mapping and dynamic whitelist filtering."""
    if df.empty: return
    
    # 1. Standardize names (dots to underscores)
    df.columns = [str(c).replace('.', '_').lower() for c in df.columns]
    
    # 2. Dynamic Whitelist Filter
    valid = get_valid_cols(table_name)
    if valid:
        df = df[[c for c in df.columns if c in valid]]
    
    # 3. Handle JSON compliance (NaN -> null)
    ...
    try:
        ...
        LOG.info(f"Sync: {len(records)} records to '{table_name}'")
    except Exception as e:
        LOG.error(f"Sync failed for '{table_name}': {e}")

def run_sync(mode="daily"):
    S_STR, S_INT = "20252026", 20252026
    LOG.info(f"--- COMPLETE LITERAL SYNC | Mode: {mode} ---")
    ...
    # 3. Discovery: Loop teams for Roster and Schedule
    ...
    # 4. Processing: Game Data (Stats & Plays)
    ...
    # 5. The "Full Identity" Step: Sync missing players found in PBP
    if all_game_stats:
        combined = pd.concat(all_game_stats)
        combined.columns = [str(c).replace('.', '_').lower() for c in combined.columns]
        
        # Discover players who skated but weren't on official team roster
...
if __name__ == "__main__":
    ...
    run_sync(parser.parse_args().mode)